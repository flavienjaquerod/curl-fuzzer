...
void fuzz_setup_http_post(FUZZ_DATA *fuzz, TLV *tlv)
{
  if (fuzz->httppost == NULL) {
    ...
    /* 1) Add simple name/content/contenttype section */
    curl_formadd(&post, &last,
      CURLFORM_COPYNAME, "html_code",
      CURLFORM_COPYCONTENTS, "<p>hello</p>",
      CURLFORM_CONTENTTYPE, "text/html; charset=UTF-8",
      CURLFORM_END);

    /* 2) Add name/ptrcontent section */
    const char *ptrname = "ptrname";
    curl_formadd(&post, &last,
      CURLFORM_PTRNAME, ptrname,
      CURLFORM_NAMELENGTH, (long)strlen(ptrname),
      CURLFORM_PTRCONTENTS, fuzz->post_body,
      CURLFORM_CONTENTSLENGTH, (curl_off_t)strlen(fuzz->post_body),
      CURLFORM_END);

    /* 3) In-memory file upload via BUFFER API */
    curl_formadd(&post, &last,
      CURLFORM_COPYNAME, "bufferfile",
      CURLFORM_BUFFER, "buffer.bin",
      CURLFORM_BUFFERPTR, fuzz->post_body,
      CURLFORM_BUFFERLENGTH, (long)strlen(fuzz->post_body),
      CURLFORM_END);

    /* 4) Guarded file path upload */
    size_t body_len = strlen(fuzz->post_body);
    if(body_len > 0 && body_len < 256 &&
      strstr(fuzz->post_body, "..") == NULL &&
      fuzz->post_body[0] != '/') {
      curl_formadd(&post, &last,
        CURLFORM_COPYNAME, "file_upload",
        CURLFORM_FILE, fuzz->post_body,
    ...
    /* 6) Trigger CURLFORM_FILENAME with custom file name */
    curl_formadd(&post, &last,
      CURLFORM_COPYNAME, "upload_with_filename",
      CURLFORM_FILE, fuzz->post_body,
      CURLFORM_FILENAME, "custom_name.txt",
      CURLFORM_END);

    /* 7) Trigger CURLFORM_FILECONTENT with file content */
    curl_formadd(&post, &last,
      CURLFORM_COPYNAME, "direct_filecontent",
      CURLFORM_FILECONTENT, fuzz->post_body,
      CURLFORM_END);

    /* 8) Trigger CURLFORM_CONTENTLEN with PTRCONTENTS */
    curl_formadd(&post, &last,
      CURLFORM_COPYNAME, "with_contentlen",
      CURLFORM_PTRCONTENTS, fuzz->post_body,
      CURLFORM_CONTENTLEN, (curl_off_t)strlen(fuzz->post_body),
      CURLFORM_END);

    /* 9) Use CURLFORM_STREAM to trigger HTTPPOST_CALLBACK case */
    char *dummy_stream_data = strdup("stream_data_placeholder");
    curl_formadd(&post, &last,
      CURLFORM_COPYNAME, "stream_input",
      CURLFORM_STREAM, dummy_stream_data,
      CURLFORM_END);
    free(dummy_stream_data);

    /* 10) Add 2 file section*/
    curl_formadd(&post, &last, 
      CURLFORM_COPYNAME, "pictures",
      CURLFORM_FILE, fuzz->post_body,
      CURLFORM_FILE, "your-face.jpg", CURLFORM_END);
    
    /* 11) Trigger CONTENTTYPE + FILENAME path */
    curl_formadd(&post, &last,
      CURLFORM_COPYNAME, "file_with_type",
      CURLFORM_FILE, fuzz->post_body,
      CURLFORM_FILENAME, "custom_type.jpg",
      CURLFORM_CONTENTTYPE, "image/jpeg",
      CURLFORM_END);

    /* 12) Trigger duplicate CONTENTTYPE with FILENAME to reach realloc branch */
    curl_formadd(&post, &last,
      CURLFORM_COPYNAME, "duplicate_type",
      CURLFORM_FILE, fuzz->post_body,
      CURLFORM_FILENAME, "with_type_again.jpg",
      CURLFORM_CONTENTTYPE, "image/png",
      CURLFORM_CONTENTTYPE, "image/jpeg",
      CURLFORM_END);

    /* 13) Trigger CONTENTHEADER */
    struct curl_slist *headers = NULL;
    headers = curl_slist_append(headers, "X-Second-Header: set");
    curl_formadd(&post, &last,
      CURLFORM_COPYNAME, "duplicated_headers",
      CURLFORM_COPYCONTENTS, fuzz->post_body,
      CURLFORM_CONTENTHEADER, headers,
      CURLFORM_END);
    curl_slist_free_all(headers);

    /* 14) Trigger this fornget call */
    curl_formget_callback cb = (curl_formget_callback)
    ([](void *arg, const char *buf, size_t len) -> size_t {
      (void)arg;
      return len;  /* pretend we consumed everything */
    });
    curl_formget(post, NULL, cb);
